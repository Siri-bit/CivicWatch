...
async function sendEmailToAuthorities(issue, authorities) {
  try {
    ...
    const subject = `URGENT: Community-Validated ${categoryDisplay} Report - ${issue.upvotes} Validations - ${issue.location.city}, ${issue.location.state}`;
...
async function composeEmailWithGemini(issue, authorities) {
  try {
    ...
    const prompt = `
You are the "Civic Issue Notification Escalator" - an automated system that notifies authorities about high-priority civic issues.

Your task is to generate an email body that MATCHES THE EXACT FORMAT below based on the provided issue details.
...
Coordinates: ${issue.location?.latitude || 0}, ${issue.location?.longitude || 0}
Image URL: ${issue.images && issue.images.length > 0 ? issue.images[0] : 'No image available'}
Reported Date: ${issue.createdAt ? issue.createdAt.toDate().toLocaleDateString() : 'Unknown'}
Mapping Link: https://www.google.com/maps?q=${issue.location?.latitude || 0},${issue.location?.longitude || 0}

=== REQUIRED EMAIL FORMAT ===
Dear [Authority Name/Department],

We are writing to report a high-priority [category] issue that has been validated by [upvotes] community members through our civic reporting system.
...
- Severity: [Severity]
- Community Validations: [Upvotes] upvotes
...
- Originally Reported: [Reported Date]

**COMMUNITY IMPACT:**
This issue has been validated by [Upvotes] different community members, indicating significant public concern and impact.

Please prioritize this repair and provide an estimated timeline for resolution.

Thank you for your attention to this matter.

Best regards,
Civic Watch Automated Notification System
`.trim();
